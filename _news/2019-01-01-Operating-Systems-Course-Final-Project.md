...
tags: ["I2C", "University Project", "Arduino", "Driver"]
---

![arduino]({{ site.baseurl }}/assets/img/i2c_videogame.jpg){: .img-center}
[![GitHub](https://img.shields.io/badge/GitHub-Code-blue)](https://github.com/lukfre/i2c_custom_driver)
{: .text-center}
...
Implementation of custom `i2c` drivers for _Master/Slave_ the communication between an `Arduino` and a generic `i2c device`.

We coded two versions:

1. `i2c/basic`: i2c communication standard primitives.
   To test these drivers, two _Master/Slave_ test mains were made between two different ATMegas _(code available in the `examples/[master/slave] folder)`_.
...
However, the display does not keep an absolute reference of the columns, but changes them with each shift. _(Ex. column 3 moves with each shift.)_
...
2. Via keyboard the Player data (_name and desired difficulty_) is entered.
